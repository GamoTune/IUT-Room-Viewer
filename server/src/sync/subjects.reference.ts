// ============================================
// 📁 src/sync/subjects.reference.ts
// Intitulés officiels des ressources et SAÉ du BUT Informatique
// ============================================

/**
 * Intitulés du programme national du BUT Informatique (PN 2022, mis à jour en
 * 2023), pour le tronc commun et le parcours A « Réalisation d'applications »,
 * seul parcours proposé à Limoges.
 *
 * Source : annexe 2 de l'arrêté du programme national, publiée par le ministère
 * de l'Enseignement supérieur. Les codes sont écrits comme dans les emplois du
 * temps de l'IUT : le `R5.Real.12` du programme y devient `R5A.12`.
 *
 * Les cases de l'IUT abrègent ces intitulés à leur façon (« NoSQL », « PPP ») et
 * n'en donnent aucun pour les cours de groupe : le référentiel fait foi, le texte
 * des cases ne sert qu'aux codes qu'il ne connaît pas.
 */
const SUBJECT_REFERENCE: Readonly<Record<string, string>> = {
    // Semestre 1
    ...
    "S3A.01": "Développement d’une application",
    ...
    "R4A.09": "Management avancé des systèmes d’information",
...
/** Le programme parle de « démarche portfolio », sans lui donner de code ; l'IUT écrit `P5A.01`. */
const PORTFOLIO_LABEL = "Démarche portfolio";

/**
 * Intitulé officiel d'un code d'emploi du temps, ou `null` s'il n'existe pas
 * dans le programme national (stage `S3.St`, SAÉ locale, mention administrative).
 */
export function officialSubjectLabel(code: string): string | null {
    if (/^P\d/.test(code)) return PORTFOLIO_LABEL;
    return SUBJECT_REFERENCE[normalizeCode(code)] ?? null;
}

/**
 * Ramène la variante de parcours à sa place habituelle : en deuxième année,
 * l'IUT écrit `S3.01A` là où la troisième écrit `S5A.01`.
 */
function normalizeCode(code: string): string {
    return code.replace(/^([RS]\d)\.(\d{2})A$/, "$1A.$2");
}
