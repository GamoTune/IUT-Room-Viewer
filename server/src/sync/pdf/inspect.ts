...
if (hours.length >= 2) {
    ...
    console.log(
        `   de ${sorted[0]!.label} (x=${sorted[0]!.center.toFixed(1)}) à ${sorted.at(-1)!.label} (x=${sorted.at(-1)!.center.toFixed(1)})`,
    );
}
...
console.log(
    "   x =",
    cluster(longVerticals.map((l) => l.x1))
        .map((v) => v.toFixed(0))
        .join("  "),
);
...
console.log(
    "   y =",
    cluster(longHorizontals.map((l) => l.y1))
        .map((v) => v.toFixed(0))
        .join("  "),
);
